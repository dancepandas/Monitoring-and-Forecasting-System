...
class AlertTracker:
    ...
    def _alert_id(self, station_code: str, alert_type: str, level: str) -> str:
        ...
        return "alert_" + hashlib.sha1(base.encode("utf-8")).hexdigest()[:16]
    ...
    def _save(self):
        try:
            ...
            tmp.write_text(json.dumps({k: v.to_dict() for k, v in self._alerts.items()}, ensure_ascii=False, indent=2, default=str), encoding="utf-8")
    ...
    def escalate(self, alert_id: str, new_level: str, new_title: str, new_message: str, metric: Optional[dict] = None) -> tuple[AlertEvent, bool]:
        """升级告警：关闭旧事件，创建新级别事件。"""
        now = time.time()
        with _lock:
    ...
    def resolve(self, alert_id: str, resolution: str = "人工解除", by: str = "") -> Optional[AlertEvent]:
        with _lock:
    ...
    def acknowledge(self, alert_id: str, by: str = "") -> Optional[AlertEvent]:
        with _lock:
            event = self._alerts.get(alert_id)
            if not event:
                return None
    ...
    def mark_notified(self, alert_id: str, channels: list[str]):
        with _lock:
            event = self._alerts.get(alert_id)
            if not event:
                return
    ...
    def list_active(self, station_code: Optional[str] = None, level: Optional[str] = None) -> list[AlertEvent]:
        with _lock:
    ...
    def get(self, alert_id: str) -> Optional[AlertEvent]:
        return self._alerts.get(alert_id)

    def cleanup_resolved(self, max_age_days: int = 7):
        """清理已恢复超过 N 天的告警。"""
